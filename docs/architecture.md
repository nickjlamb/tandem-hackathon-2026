...
- **Safety-net checks** (`tracker.run_checks`), run after every event and clock tick: result overdue; appointment at risk (an outstanding item is overdue or on hold within 14 days of the appointment, or its expected date + buffer is after the appointment); appointment passed while the loop is open; **second-line escalation** — an alert unactioned for `ESCALATE_AFTER_DAYS` (5) moves from the requesting clinician to the clinic consultant, and after another 5 days to the service lead.
- **Worklist** (`tracker.worklist`): every open loop scored on a fixed ladder (appointment imminent with results outstanding > result overdue > on hold > appointment passed > on track > pack ready), +20 per escalation tier, sorted by score then age. `seed.py` builds a six-patient synthetic clinic by walking the clock forward a day at a time through these same rules.
